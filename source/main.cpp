...
void help(){
    ...
    std::cout << "Usage example: cleanFasta2 -infile infile.fas -outfile outfile.fas -maxMissing 50 [-verbose 1] [ -winsize 1000000 ] [ -samples samples.txt ] [-format fasta/vcf/stats] [-gff annot.gff -scaffold scaf1 -include 0 -feature repeat_region -perFeature 0] [-mergeCDS 0] [-strictNames 1] [-onlyVars 0]" << std::endl;
    ...
    std::cout << "-format: output format, either fasta (the default), vcf or stats." << std::endl;
    std::cout << "-strictNames: if set to 1 (the default), will exit program with an error if not all samples in -samples file are found in infile. If set to 0, will just raise a warning and continue." << std::endl;
    std::cout << "-onlyVars: if set to 0 (the default) will output all sites that pass filters. If set to 1 will output only sites that are variable (within -samples if defined). Only available for fasta and vcf outputs." << std::endl << std::endl;
...
//130125: added option to output just variable positions
//150125: fixed vcf output, quality column was missing
...
int main(int argc, const char * argv[]) {
    ...
    bool onlyVars = programOptions.isArgDefined("onlyVars") ? programOptions.getBool("onlyVars") : false;
    ...
    if(onlyVars && outputFormat ==  "stats" ){
        std::cerr << "For onlyVars option no stats output available" << std::endl;
        exit(1);
    }   
    ...
    mySeqHandler.setVarsOnly(onlyVars);
